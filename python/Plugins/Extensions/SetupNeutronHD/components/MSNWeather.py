...
#degreetype = 'F'
degreetype = 'C'
#weather_location = 'en-EN'
weather_location = 'ru-RU'
#weather_city = 'Kiev,Kiev,Ukraine'
#weather_city = 'Moscow,Moscow-City,Russia'
weather_city = 'Aleksandrov,Vladimir,Russia'
time_update = 20
time_update_ms = 3000

class MSNWeather(Poll, Converter, object):
	...
	def __init__(self, type):
		...
		if type == "Format":
			...
			self.type = self.WIND
		elif type == "Windspeed_ms":
			self.type = self.WINDSPEED_MS
		elif type == "Windspeed_km":
			self.type = self.WINDSPEED_KM
#	today	#
		elif type == "Date0":
...
#	day 4	#
	...
	def write_none(self):
		with open('/tmp/msnweather.xml', 'w') as noneweather:
			noneweather.write('None')
		noneweather.close()

	def get_xmlfile(self):
		self.iConsole.ePopen("wget -P /tmp -T2 'http://weather.service.msn.com/data.aspx?weadegreetype=%s&culture=%s&weasearchstr=%s' -O /tmp/msnweather.xml" % (degreetype, weather_location, weather_city), self.control_xml)
	...
	def getText(self):
		...
		msnweather = {'Format':'', 'Date':'', 'Shortdate':'', 'Day':'', 'Shortday':'',\
			...
			'Temp':'', 'Picon':'', 'Skytext':'', 'Feelslike':'', 'Humidity':'', 'Wind':'', 'Windspeed_ms':'', 'Windspeed_km':'',\
		...
		if fileExists("/tmp/msnweather.xml"):
			if int((time.time() - os.stat('/tmp/msnweather.xml').st_mtime)/60) >= time_update:
				self.get_xmlfile()
		else:
			self.get_xmlfile()
		if not fileExists('/tmp/msnweather.xml'):
			self.write_none()
			return info
		if fileExists('/tmp/msnweather.xml') and open('/tmp/msnweather.xml').read() is 'None':
			return info
		for line in open("/tmp/msnweather.xml"):
			if "<weather" in line:
				try:
					msnweather['Location'] = line.split('weatherlocationname')[1].split('"')[1]
			...
			if "<current" in line:
				try:
					...
					msnweather['Humidity'] = line.split('humidity')[1].split('"')[1] + ' %'
					msnweather['Wind'] = line.split('winddisplay')[1].split('"')[1].split(' ')[2]
					msnweather['Windspeed_ms'] = '%3.02f m/s' % ((float(line.split('windspeed')[1].split('"')[1]) * 1000)/3600)
					msnweather['Windspeed_km'] = '%s km/h' % line.split('windspeed')[1].split('"')[1]
...
#	today	#
			if "<forecast" in line:
				try:
					...
					msnweather['Picon0'] = line.split('skycode')[2].split('"')[1]
					...
					msnweather['Precip0'] = line.split('precip')[1].split('"')[1] + ' %'
				except:
					pass
#	day 1	#
			if "<forecast" in line:
				try:
					...
					msnweather['Picon1'] = line.split('skycode')[3].split('"')[1]
					...
					msnweather['Precip1'] = line.split('precip')[2].split('"')[1] + ' %'
				except:
					pass
#	day 2	#
			if "<forecast" in line:
				try:
					...
					msnweather['Picon2'] = line.split('skycode')[4].split('"')[1]
					...
					msnweather['Precip2'] = line.split('precip')[3].split('"')[1] + ' %'
				except:
					pass
#	day 3	#
			if "<forecast" in line:
				try:
					...
					msnweather['Picon3'] = line.split('skycode')[5].split('"')[1]
					...
					msnweather['Precip3'] = line.split('precip')[4].split('"')[1] + ' %'
				except:
					pass
#	day 4	#
			if "<forecast" in line:
				try:
					...
					msnweather['Picon4'] = line.split('skycode')[6].split('"')[1]
					...
					msnweather['Precip4'] = line.split('precip')[5].split('"')[1] + ' %'
				except:
					pass
#
		if self.type is self.FORMAT:
			info = msnweather['Format']
		...
		if self.type is self.WINDSPEED_MS:
			info = msnweather['Windspeed_ms']
		if self.type is self.WINDSPEED_KM:
			info = msnweather['Windspeed_km']
#	today	#
